 ...
 * 
 * <p>
 * This class is useful if you have simple needs and wish to use the DESede encryption cipher. More sophisticated
 * requirements will need to use the Java crypto libraries directly.
 * 
 * @author Alan Stewart
 * @author Ben Alex
 */
...
public final class EncryptionUtils {
	...
	/**
	 * Converts a String into a byte array using UTF-8, falling back to the platform's default character set if UTF-8
	 * fails.
	 * 
	 * @param input
	 *            the input (required)
	...
	/**
	 * Converts a byte array into a String using UTF-8, falling back to the platform's default character set if UTF-8
	 * fails.
	 * 
	 * @param byteArray
	...
	public static String encrypt(String key, String inputString) throws EncryptionException {
		...
		return byteArrayToString(Base64Utils.encode(cipherText));
	}
	...
	public static byte[] encrypt(String key, byte[] inputBytes) throws EncryptionException {
		isValidKey(key);
		return Base64Utils.encode(cipher(key, inputBytes, Cipher.ENCRYPT_MODE));
	}
	...
	public static String decrypt(String key, String inputString) throws EncryptionException {
		...
		final byte[] cipherText = cipher(key, Base64Utils.decode(stringToByteArray(inputString)), Cipher.DECRYPT_MODE);
	...
	public static byte[] decrypt(String key, byte[] inputBytes) throws EncryptionException {
		Assert.hasText(key, "A key is required to attempt decryption");
		return cipher(key, Base64Utils.decode(inputBytes), Cipher.DECRYPT_MODE);
	}
