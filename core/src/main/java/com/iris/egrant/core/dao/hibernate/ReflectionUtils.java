...
public class ReflectionUtils {
	...
	public static Serializable getId(Object obj) /*throws ServiceException*/ {
		...
		try {
			Assert.notNull(obj, "acl domain不允许为空！");
			Field[] fields = obj.getClass().getDeclaredFields();
			Serializable entityKey = null;

			for (Field field : fields) {
				boolean isEntityKey = field.isAnnotationPresent(Id.class)
						|| field.isAnnotationPresent(EmbeddedId.class);
				if (isEntityKey) {

					Object key = obj.getClass()
							.getMethod("get" + StringUtils.capitalize(field.getName()), new Class[] {})
							.invoke(obj, new Class[] {});

					entityKey = new Long(key.toString());
			...
			if (entityKey == null) {
				...
				for (Method method : methods) {
					...
					boolean isEntityKey = method.isAnnotationPresent(Id.class)
							|| method.isAnnotationPresent(EmbeddedId.class);
					if (isEntityKey) {

						entityKey = new Long(method.invoke(obj, new Class[] {}).toString());
			...
			logger.error("读取acl domain={}的Id失败！", obj, e);
